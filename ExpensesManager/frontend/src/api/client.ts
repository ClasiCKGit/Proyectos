  ...
  if (res.status === 401) {
    ...
    if (body?.code === "TOKEN_EXPIRED") {
      ...
      if (newToken) {
        ...
        if (!retry.ok) throw Object.assign(new Error(retryData?.message ?? "Error"), { status: retry.status });
        return retryData as T;
      }
    }
    throw Object.assign(new Error(body?.message ?? "No autorizado"), { status: 401 });
  }
...
export const transactionsApi = {
  ...
  create: (body: unknown) => request<any>("/transactions", { method: "POST", body: JSON.stringify(body) }),
  update: (id: string, body: unknown) => request<any>(`/transactions/${id}`, { method: "PATCH", body: JSON.stringify(body) }),
...
export const savingsGoalsApi = {
  ...
  create: (body: unknown) => request<any>("/savings-goals", { method: "POST", body: JSON.stringify(body) }),
  update: (id: string, body: unknown) => request<any>(`/savings-goals/${id}`, { method: "PATCH", body: JSON.stringify(body) }),
  contribute: (id: string, amount: number) =>
    request<any>(`/savings-goals/${id}/contribute`, { method: "POST", body: JSON.stringify({ amount }) }),
  remove: (id: string) => request<void>(`/savings-goals/${id}`, { method: "DELETE" }),
};
