...
import type { RecurringTransaction, UpcomingOccurrence } from "../api/recurring";
...
const EMPTY_FORM: FormData = {
  type: "expense", amount: "", description: "", category: "savings",
  recurrence: "monthly",
...
const RecurringForm: React.FC<{
  ...
  const [form, setForm] = useState<FormData>(
    initial ? {
      type: initial.type, amount: String(initial.amount),
      description: initial.description, category: initial.category as Category,
      recurrence: initial.recurrence, startDate: initial.startDate,
      endDate: initial.endDate ?? "", tags: initial.tags.join(", "),
      notes: initial.notes ?? "",
    } : { ...EMPTY_FORM }
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSubmit({
        ...
        category: form.category, recurrence: form.recurrence,
    ...
    <div className={styles.ffWrapper}>
      ...
      <div className={styles.typeToggle}>
        {(["expense", "income"] as const).map((t) => (
          <button key={t} className={ `${styles.typeBtn} ${form.type === t ? styles.typeBtnActive : ""}` }
      ...
      <div className={styles.grid}>
        ...
        <div className={styles.field}>
          ...
          <select className={styles.input} value={form.recurrence}
            onChange={(e) => setForm((p) => ({ ...p, recurrence: e.target.value as FormData["recurrence"] }))}>
        ...
        <div className={styles.field} style={{ gridColumn: "1 / -1" }}>
          ...
          <input className={styles.input} type="text" placeholder="Ej: Alquiler, Netflix, Sueldo..."
            value={form.description} onChange={set("description")} maxLength={100} />
        </div>

        {form.type === "expense" && (<div className={styles.field}>
        ...
        <div className={styles.field} style={form.type !== "expense" ? {gridColumn: "1 / -1"} : {}}>
          ...
          <input className={styles.input} type="text" placeholder="fijo, servicios..."
        ...
        <div className={styles.field}>
          <label className={styles.label} style={form.type !== "expense" ? {gridColumn: "1 / -1"} : {}}>Fecha de fin (opcional)</label>
      ...
      <div className={styles.actions}>
        <button className={ styles.btnPrimary }style={{ opacity: loading ? 0.7 : 1 }}
          onClick={handleSubmit} disabled={loading}>
          {loading ? "Guardando..." : initial ? "Guardar cambios" : "Crear recurrencia"}
        </button>
        <button className={styles.btn} onClick={onCancel}>Cancelar</button>
      </div>
    </div>
  );
};
...
const RecurringCard: React.FC<{
  ...
  const handleToggle = async () => {
    ...
    try { await onToggle(item.id); } finally { setLoadingToggle(false); }
  };

  const handleDelete = async () => {
    if (!confirm(`¿Eliminar "${item.description}"?`)) return;
    setLoadingDelete(true);
    try { await onDelete(item.id); } finally { setLoadingDelete(false); }
  };
    ...
    <div className={styles.card} style={{ opacity: item.isActive ? 1 : 0.55 }}>
      <div className={styles.header}>
        <div className={styles.left}>
          ...
          <div>
            ...
            <div className={styles.meta}>
              ...
              <span style={{ color: item.type === "income" ? "#1D9E75" : "inherit" }}>
        ...
        <div className={styles.right}>
          <span className={styles.badge}>
            {RECURRENCE_ICONS[item.recurrence]} {RECURRENCE_LABELS[item.recurrence]}
          </span>
        </div>
      </div>
      ...
      <div className={styles.footer}>
        <div className={styles.footerLeft}>
          ...
          {item.endDate && (
            <span className={styles.footerText}>· Fin: {formatDate(item.endDate)}</span>
          )}
        ...
        <div className={styles.actions}>
          <button className={styles.actionBtn} style={{ color: item.isActive ? "#BA7517" : "#1D9E75" }}
            onClick={handleToggle} disabled={loadingToggle}
            title={item.isActive ? "Pausar" : "Activar"}>
            {loadingToggle ? "..." : item.isActive ? "⏸" : "▶"}
          </button>
          <button className={styles.actionBtn} onClick={() => onEdit(item)} title="Editar">✎</button>
          <button className={styles.actionBtn} style={{ color: "#D85A30" }}
            onClick={handleDelete} disabled={loadingDelete} title="Eliminar">
            {loadingDelete ? "..." : "✕"}
          </button>
        </div>
      </div>
...
const UpcomingCalendar: React.FC<{ upcoming: UpcomingOccurrence[] }> = ({ upcoming }) => {
  ...
  return (
    <div className={styles.ucWrapper}>
      {Object.entries(grouped).map(([weekKey, items]) => {
        ...
        const totalExpenses = items.filter((i) => i.type === "expense").reduce((a, i) => a + i.amount, 0);
        const totalIncome = items.filter((i) => i.type === "income").reduce((a, i) => a + i.amount, 0);
          ...
          <div key={weekKey} className={styles.week}>
            <div className={styles.weekHeader}>
              ...
              <div className={styles.weekTotals}>
                {totalIncome > 0 && <span style={{ color: "#1D9E75", fontSize: 12 }}>+{formatCurrency(totalIncome)}</span>}
                {totalExpenses > 0 && <span style={{ color: "#D85A30", fontSize: 12 }}>−{formatCurrency(totalExpenses)}</span>}
              </div>
            </div>
            {items.map((o, i) => {
              ...
              return (
                <div key={i} className={styles.item}>
                  <span className={styles.itemDate}>{formatDate(o.date, { day: "2-digit", month: "short" })}</span>
                  <span className={styles.itemIcon}>{CATEGORY_ICONS[category]}</span>
                  <span className={styles.itemDesc}>{o.description}</span>
                  <span className={ styles.itemAmount} style={{ color: o.type === "income" ? "#1D9E75" : "inherit" }}>
...
export const RecurringTab: React.FC<RecurringTabProps> = ({
  items, upcoming, newlyGenerated,
  onAdd, onEdit, onToggle, onDelete, onDismissNotifications,
}) => {
    ...
    <div className={styles.msWrapper}>
        ...
        <div className={styles.summary}>
          ...
          <div className={styles.summaryItem}>
            ...
            <span className={styles.summaryValue} style={{ color: "#D85A30" }}>
              {formatCurrency(
                active.filter((r) => r.type === "expense" && r.recurrence === "monthly")
          ...
          <div className={styles.summaryItem}>
            ...
            <span className={styles.summaryValue} style={{ color: "#1D9E75" }}>
              {formatCurrency(
                active.filter((r) => r.type === "income" && r.recurrence === "monthly")
                  .reduce((a, r) => a + r.amount, 0)
              )}
            </span>
          </div>
      ...
      <div className={styles.controls}>
        <div className={styles.viewToggle}>
          ...
          <button className={ `${styles.viewBtn} ${view === "calendar" ? styles.viewBtnActive : ""}` }
            onClick={() => setView("calendar")}>Próximos 30 días</button>
        </div>
        <button className={styles.addBtn}
          onClick={() => { setShowForm(true); setEditingItem(undefined); }}>
          + Nueva recurrencia
        </button>
      </div>
      ...
      {(showForm || editingItem) && (
        <div className={styles.formCard}>
          <p className={styles.formTitle}>{editingItem ? "Editar recurrencia" : "Nueva recurrencia"}</p>
          <RecurringForm
            initial={editingItem}
      ...
      {view === "list" ? (
        items.length === 0 ? (
          ...
          <div className={styles.list}>
            {active.length > 0 && (
              <>
                <div className={styles.sectionLabel}>Activas ({active.length})</div>
                {active.map((r) => (
            ...
            {paused.length > 0 && (
              <>
                <div className={ styles.sectionLabel } style={{ marginTop: 12 }}>Pausadas ({paused.length})</div>
                {paused.map((r) => (
                  <RecurringCard key={r.id} item={r}
                    onEdit={(item) => { setEditingItem(item); setShowForm(false); }}
                    onToggle={onToggle}
        ...
        upcoming.length === 0
          ? <div className={styles.empty}>No hay vencimientos en los próximos 30 días.</div>
